...
Himanshu 29%
  */
object PassFailPercentage extends App{
  ...
  val passed=passFail(l2,l1,"pass",65)
  println("The passed students are: ")
  passed.map(println _)
  val failed=passFail(l2,l1,"Fail",65)
  println("The failed students are: ")
  failed.map(println _)

  def passFail(marksList:List[Marks],studentsList:List[Student],choice:String,percent:Int):List[(String,Int)]={
